          ...
          <table>
            <thead>
              <tr className="title-row">
                <th colSpan={6}>{rows[0]?.values![0].formattedValue}</th>
              </tr>
              <tr className="heading-row">
                {rows[1]?.values?.slice(0, 6).map((h: sheets_v4.Schema$CellData, index: number) =>
                  <th key={index}>
                    {h.formattedValue}
                  </th>
                )}
            ...
            <tbody>
              <tr className="player-1">
                {rows[2]?.values?.slice(0, 6).map((s: sheets_v4.Schema$CellData, index: number) =>
                  <td key={index}>
                    {index === 0 &&
              ...
              <tr className="player-2">
                {rows[3]?.values?.slice(0, 6).map((s: sheets_v4.Schema$CellData, index: number) =>
                  <td key={index}>
                    {index === 0 &&
                      <strong>{s.formattedValue}</strong>
                    }
